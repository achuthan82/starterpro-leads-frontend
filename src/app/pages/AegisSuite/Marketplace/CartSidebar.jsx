...
const CartSidebar = ({ open, onClose }) => {
  const { cart, loading, error, updateCartItem, removeFromCart, refreshCart } =
    useCart();
  ...
  const handleUpdate = async (id) => {
    ...
    if (!item || isNaN(newQty) || newQty < 1 || newQty === item.quantity)
      return;
    ...
    if (stock && stock[id] && typeof stock[id].stock === "number") {
      if (newQty > stock[id].stock) {
        setStockError((e) => ({
          ...e,
          [id]: `Only ${stock[id].stock} available.`,
        }));
        return;
      }
    }
  ...
  const roundedTotalWithCommission = toFloatWithoutRounding(
    grandTotal + commission,
    2,
  ); // Math.round(grandTotal + commission);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (reserveData && reserveData.data && !Array.isArray(reserveData.data)) {
        ...
        cart.forEach((item) => {
          if (
            ...
            newStockError[item.id] =
              `Only ${reserveData.data[item.id].stock} available.`;
          }
        });
      ...
      const commission = toFloatWithoutRounding(
        (grandTotal * 0.04 * 100) / 100,
        3,
      ); // Math.round(grandTotal * 0.03 * 100) / 100;
      const roundedTotalWithCommission = toFloatWithoutRounding(
        grandTotal + commission,
        2,
      ); // Math.round(grandTotal + commission)
      ...
      sessionStorage.setItem(
        ...
        stripeData.data.invoice_number,
      );
      const stripe = await loadStripe(STRIPE_KEY);
      if (stripe) {
        await stripe.redirectToCheckout({
          sessionId: stripeData.data.session_id,
        });
      } else {
      ...
      <div className="flex h-full flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
            ...
            <div className="rounded-md bg-red-100 p-4 text-red-500 dark:bg-red-900/20 dark:text-red-400">
              {error}
            </div>
          ) : cart.length === 0 ? (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      disabled={
                        ...
                        isNaN(pendingQuantities[item.id]) ||
                        Number(pendingQuantities[item.id]) < 1 ||
                        loading
                      }
